-- Store Keepers maintain the item master at their assigned site, so they may
-- fold an obvious duplicate into the correct item. The function still checks
-- both items belong to one site and that the caller can access that site.
